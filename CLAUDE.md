...
- Los CSV de `splits/` son derivados reproducibles (`make splits` / `make splits-baseline`). No editarlos a mano. Viven en `data/splits/` (ver más abajo), no bajo `DATASET_ROOT`.

## Arquitectura (`src/`)

- **Único punto de entrada a imagen:** `load_and_normalize_image()` (`src/data/loader.py`).
- **Rutas:** dataset fuente vía `get_dataset_root()`, datos derivados reproducibles vía
  `get_project_data_root()` y resultados vía `get_output_root()` (todas en `src/config.py`).
  Nunca hardcodear paths ni usar la constante `DATASET_ROOT` directo.
...
- **Datos:** `clean/<clase>/{lab,real}/` → `create_splits.py` (valida integridad PIL, deduplica por SHA-256 con escaneo `sorted()` - determinista entre máquinas -, estratifica por `label+environment`) → `data/splits/seed_42/` (9 clases) o `data/splits/seed_42_baseline/` (`--baseline`, subset de `config/dataset.yaml -> baseline:`).
- **Orden de artefactos:** `data/` contiene splits, pilotos, imágenes copiadas y anotaciones;
  `outputs/` contiene modelos, métricas, auditorías, previews, validaciones y diagnósticos.
